...
public class UseSHA {
	...
	public static void main(String[] args) {
		...
		try {
			...
			//String sha = new String(shaArr); 사람이 못 알아보면 안되니깐 사람이 알아 볼 수 있는 문자열로 만듦.
			//4. 문자열로 생성 (사람이 알아 볼 수 없다. > 사람이 알아 볼 수 있는 문자열로)
			String sha = new String(Base64.getEncoder().encodeToString(shaArr));
			String sha2 = new String(Base64.getEncoder().encodeToString(shaArr2));
			String sha3 = new String(Base64.getEncoder().encodeToString(shaArr3));
			System.out.println("평문 : " + plainText);
			System.out.println("MD5 : " + sha);
			System.out.println("SHA-1 : " + sha2);
			System.out.println("SHA-512 : " + sha3);
			
			//강사님이 주신 jar파일인데, 위의 작업을 만들어둔 것.
