...
<body>
<%
//1.쿠키들 읽기 : JSESSIONID는 tomcat에서 자동으로 생성하는 쿠키.(무조건 존재)
...
if(cookies != null) {//읽어 들인 쿠키가 존재?
	...
	for(int i=0; i<cookies.length; i++) {
		...
		if(!"JSESSIONID".equals(tempCookie.getName())){
			...
			flag=true;
			cookieName.add(tempCookie.getName());//쿠키의 이름 
			cookieValue.add(tempCookie.getValue());//쿠키의 값
		}//end if
	}//end for
}//end if
%>
...
<strong>쿠키 사용</strong>
					<form name="frm" id="frm" action="use_cookie_b.jsp">
						...
						<td rowspan="2"><input type="button" value="입력" id="btn" class="btn btn-info" style="height:68px; width:80px;" tabindex="3"/></td>
						</tr>
						<tr>
