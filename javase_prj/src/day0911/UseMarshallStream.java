...
public class UseMarshallStream {
	...
	public void objectOutput(UserData ud) {
		...
		try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(file))) {
			...
			System.err.println("직렬화 문제 발생");
			ne.printStackTrace();
		} catch (IOException ie) {
			System.err.println("입출력 문제 발생");
			ie.printStackTrace();
		} // end catch
	...
	public void objectInput() {
		...
		try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(file))) {
			...
			UserData ud = (UserData) ois.readObject();// 부모 -> 자식으로 캐스팅 해서 넣어줌.
			System.out.println("읽어들인 객체 : ");
			System.out.println(ud.getName() + " / " + ud.getWeight() + " / " + ud.getHeigth());
		} catch (ClassNotFoundException cnfe) {
			System.out.println("읽어들인 파일에 객체가 없음.");
			cnfe.printStackTrace();
		} catch (IOException ie) {
			System.out.println("입출력예외 객체가 없음.");
			ie.printStackTrace();
		} // end catch
	...
	public static void main(String[] args) {
		...
		ums.objectOutput(ud);//객체 쓰기
		
		ums.objectInput();//객체 읽기
	}// main
